 ...
 * - fields: [{key,label,type,help,options?,placeholder?,default?}]
 */
export default function SettingsEditor({ title, prefix, fields }) {
  ...
  useEffect(() => {
    (async () => {
      setErr("");
      try {
        // ✅ FIXED: this must be /settings (your backend mounts /api/settings/*)
  ...
  const save = async () => {
    ...
    try {
      ...
      // backend: PUT /api/settings expects an array of {key,value}
