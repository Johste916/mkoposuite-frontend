...
/** Generic CRUD for /admin/types/:category (name/code + optional meta JSON) */
export default function TypesEditor({ title, category }) {
  ...
  const add = async () => {
    try {
      ...
      } else if (form.meta && typeof form.meta === "object") {
        meta = form.meta;
      }
  ...
  const del = async (id) => {
    ...
    try {
      await api.delete(`/admin/types/${category}/${id}`);
      await load();
    } catch (e) {
      setErr(e?.response?.data?.error || e.message);
    }
      ...
      <div className="grid md:grid-cols-3 gap-2 items-end">
        ...
        <div className="md:col-span-3">
          ...
          <textarea
            ...
            className="w-full border rounded px-2 py-1 text-sm"
            value={form.meta}
            onChange={(e) => setForm((s) => ({ ...s, meta: e.target.value }))}
          />
        </div>
        <div>
