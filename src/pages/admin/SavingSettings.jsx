...
export default function SavingSettings() {
  const { data, setData, loading, saving, error, success, save } =
    useSettingsResource(SettingsAPI.getSavingSettings, SettingsAPI.saveSavingSettings, {
      minBalance:0, allowOverdraft:false, interestRate:0
    });
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border rounded-2xl p-4 space-y-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div><label className="text-sm">Min Balance</label>
          <input type="number" className="mt-1 w-full rounded border px-3 py-2 text-sm"
                 value={data.minBalance ?? 0} onChange={(e)=>setData({...data, minBalance:Number(e.target.value)})}/>
        </div>
        <div><label className="text-sm">Interest Rate (%)</label>
          <input type="number" className="mt-1 w-full rounded border px-3 py-2 text-sm"
                 value={data.interestRate ?? 0} onChange={(e)=>setData({...data, interestRate:Number(e.target.value)})}/>
        </div>
        <label className="flex items-center gap-2 md:col-span-2">
          <input type="checkbox" checked={!!data.allowOverdraft}
