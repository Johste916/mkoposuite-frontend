...
export default function Signup() {
  ...
  const [status, setStatus] = useState({ enabled: true, defaultTrialDays: 14, requireEmailVerification: false });
  ...
  useEffect(() => {
    let mounted = true;
    api.get('/signup/status').then(({ data }) => {
      if (!mounted) return;
      setStatus({
  ...
  async function submit(e) {
    e.preventDefault();
    if (!status.enabled) { toast.error('Self-signup is disabled'); return; }
    setLoading(true);
    try {
      ...
      const msg = err?.response?.data?.error || err.message || 'Failed to sign up';
      toast.error(msg);
    } finally {
      setLoading(false);
    }
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow p-6">
        ...
        <form onSubmit={submit} className="space-y-4">
          <div>
            ...
            <input
              name="companyName" value={form.companyName} onChange={onChange} required
          ...
          <div>
            ...
            <select
              name="planCode" value={form.planCode} onChange={onChange}
