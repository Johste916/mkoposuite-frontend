...
const SECTIONS = [
  ...
  { title: "Loans", icon: <FiCreditCard className="opacity-70" />, items: [
      "Loan Products","Loan Penalty Settings","Loan Fees","Loan Repayment Cycles",
      "Loan Reminder Settings","Loan Templates: Applications/Agreements","Manage Loan Status and Approvals"
    ]},
  { title: "Manage Branches", icon: <FiList className="opacity-70" />, items: [
      "Branches","Branch Holidays"
    ]},
...
export default function Admin() {
  ...
  const filtered = useMemo(() => {
    ...
    return SECTIONS
      .map((sec) => ({
        ...sec,
        items: sec.items.filter((label) => label.toLowerCase().includes(term)),
      }))
