...
export default function BankTransfers() {
  ...
  const [amount,setAmount]=useState(""); const [occurredAt,setOccurredAt]=useState(""); const [reference,setReference]=useState("");
  useEffect(()=>{ api.get("/banks").then(r=>setBanks(r.data||[])); },[]);
  const submit=async(e)=>{ e.preventDefault(); if(!from||!to||!amount) return alert("From, to, amount");
    await api.post(`/banks/${from}/transfer`,{toBankId:to,amount:Number(amount),occurredAt,reference});
        ...
        <div className="grid md:grid-cols-2 gap-3">
          ...
          <div><label className="text-xs text-gray-600">To</label>
            <select className={cls} value={to} onChange={e=>setTo(e.target.value)}>
              <option value="">—</option>{banks.filter(b=>b.id!==from).map(b=><option key={b.id} value={b.id}>{b.name}</option>)}
            </select></div>
          ...
          <div><label className="text-xs text-gray-600">Occurred At</label>
            <input className={cls} type="datetime-local" value={occurredAt} onChange={e=>setOccurredAt(e.target.value)} /></div>
          <div className="md:col-span-2"><label className="text-xs text-gray-600">Reference</label>
            <input className={cls} value={reference} onChange={e=>setReference(e.target.value)} /></div>
        </div>
        <div className="mt-4"><button className="px-4 py-2 rounded bg-indigo-600 text-white">Transfer</button></div>
      </form>
    </div>
  );
}
